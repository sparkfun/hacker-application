...
namespace AspDotNetStorefrontAdmin
{
	...
	public class lat_account : SkinBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			rs.Close();
			SectionTitle = "<a href=\"affiliates.aspx?searchfor=" + AffiliateID.ToString() + "\">Affiliates</a> - Account Summary Page For: " + (FirstName + " " + LastName).Trim() + " (" + EMail + ")" + ")";
		}
...
//			writer.Write("        </table>\n");
			...
			if(!TrackingOnly)
			{
				...
				writer.Write("<p><b>To see the " + Common.AppConfig("AffiliateProgramName") + " FAQs, <a href=\"lat_driver.aspx?affiliateid=" + AffiliateID.ToString() + "&topic=affiliate_faq\">click here</a>.</b></p>\n");
			}
			...
			IDataReader rs = DB.GetRS("select * from affiliate where affiliateid=" + Common.QueryStringUSInt("AffiliateID"));
			rs.Read();
			...
			String act = "lat_account_process.aspx?affiliateid=" + AffiliateID.ToString();
			...
			if(Common.QueryString("updated").Length != 0)
			{
				writer.Write("<small>&nbsp;<b><font color=blue>(UPDATED)</font></b></small>");
			}
			writer.Write("<br>");
			...
			writer.Write("        <input type=\"text\" name=\"Password\" size=\"37\" maxlength=\"100\" value=\"" + Server.HtmlEncode(PWD) + "\"> (required)");
			...
			writer.Write("        <td width=\"75%\">");
			writer.Write("        <input type=\"text\" name=\"Password2\" size=\"37\" maxlength=\"100\" value=\"" + Server.HtmlEncode(PWD) + "\"> (required)");
			...
			writer.Write("        <td width=\"75%\">");
			writer.Write("        <input type=\"text\" name=\"DefaultSkinID\" size=\"3\" maxlength=\"3\" value=\"" + Server.HtmlEncode(DB.RSFieldInt(rs,"DefaultSkinID").ToString()) + "\"> (required)");
