...
//			int NN = DB.GetSqlN("select count(*) as N from Affiliate  " + DB.GetNoLock() + " where EMail=" + Common.SQuote(EMailField) + " and AffiliateID<>" + thisCustomer._affiliateID.ToString());
//			if(NN > 0)
//			{
			...
			if(ErrorMsg.Length == 0)
			{
				...
				try
				{
					...
					if(!Editing)
					{
						...
						sql.Append("insert Affiliate(AffiliateGUID,EMail,[Password],DateOfBirth,Gender,TrackingOnly,IsOnline,DefaultSkinID,FirstName,LastName,Name,Company,Address1,Address2,Suite,City,State,Zip,Country,Phone,WebSiteName,WebSiteDescription,URL) values(");
						...
						sql.Append(Common.Form("DefaultSkinID") + ",");
				...
				{
					ErrorMsg = "<p><b>ERROR: There was an unknown error in adding the account record.<br><br></b></p>";
				}

			}

			String AffiliateGUID = String.Empty;
			IDataReader rsx = DB.GetRS("select AffiliateGUID from affiliate where AffiliateID=" + AffiliateID.ToString());
			rsx.Read();
...
//			writer.Write("                                • <a href=\"lat_earnings.aspx?affiliateid=" + AffiliateID.ToString() + "\">Your Earnings</a><br>\n");
			...
			else
			{
				try
				{
					...
					Common.SendMail("" + Common.AppConfig("AffiliateProgramName") + " New Member Notification",FormContents,true,Common.AppConfig("MailMe_FromAddress"),Common.AppConfig("MailMe_FromName"),Common.AppConfig("RewardsEmailAddress"),Common.AppConfig("RewardsEmailAddress"),"webmaster@AspDotNetStorefront.com",Common.AppConfig("MailMe_Server"));
				}
				catch {}
				...
				try
				{
					...
					Common.SendMail("" + Common.AppConfig("AffiliateProgramName") + " Welcome",WelcomeMsg,true,Common.AppConfig("RewardsEmailAddress"),Common.AppConfig("RewardsEmailAddress"),EMailField,EMailField,"",Common.AppConfig("MailMe_Server"));
				}
				catch {}
				...
				if(Common.Form("TrackingOnly") != "1")
				{
					//Common.AssignCardToAffiliate(thisCustomer._customerID,AffiliateID,AffiliateID.ToString(),(decimal)Common.AppConfigUSSingle("GiftCardAmount"),1,System.DateTime.Now,System.Localization.ParseUSDateTime("12/31/2050"));
					//DB.ExecuteSQL("insert AffiliateCardRequest(AffiliateID,RequestedNumber) values(" + AffiliateID.ToString() + ",20)");
				...
				{
					writer.Write("<p align=\"left\"><b>TRACKING affiliate sign-up was successful.<br><br><a href=\"lat_account.aspx?affiliateid=" + AffiliateID.ToString() + "\">Click here</a> to go to your account page.</p>");
				}
			}
