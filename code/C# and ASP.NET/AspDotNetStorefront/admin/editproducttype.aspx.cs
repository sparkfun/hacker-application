...
namespace AspDotNetStorefrontAdmin
{
	...
	/// </summary>
	public class editproducttype : SkinBase
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			if(Common.QueryString("producttypeID").Length != 0 && Common.QueryString("producttypeID") != "0") 
			{
				Editing = true;
				producttypeID = Localization.ParseUSInt(Common.QueryString("producttypeID"));
			} 
			else 
			{
				Editing = false;
			...
			if(Common.Form("IsSubmit").ToUpper() == "TRUE")
			{
				...
				if(Editing)
				{
					// see if this producttype already exists:
					rs = DB.GetRS("select count(name) as N from producttype  " + DB.GetNoLock() + " where producttypeID<>" + producttypeID.ToString() + " and deleted=0 and lower(Name)=" + DB.SQuote(Common.Form("Name").ToLower()));
					rs.Read();
					N = DB.RSFieldInt(rs,"N");
					rs.Close();
					if(N != 0)
					{
						ErrorMsg = "<p><b><font color=red>ERROR:<br><br></font><blockquote>There is already another producttype with that name. Please <a href=\"javascript:history.back(-1);\">go back</a>.</b></blockquote></p>";
					}
				}
				else
				{
					// see if this name is already there:
					rs = DB.GetRS("select count(name) as N from producttype  " + DB.GetNoLock() + " where deleted=0 and lower(Name)=" + DB.SQuote(Common.Form("Name").ToLower()));
					rs.Read();
					N = DB.RSFieldInt(rs,"N");
					rs.Close();
					if(N != 0)
				...
				if(ErrorMsg.Length == 0)
				{
					try
					{
						...
						if(!Editing)
						{
							...
							sql.Append("insert into producttype(producttypeGUID,Name,LastUpdatedBy) values(");
							...
							rs = DB.GetRS("select producttypeID from producttype  " + DB.GetNoLock() + " where deleted=0 and producttypeGUID=" + DB.SQuote(NewGUID));
							rs.Read();
		...
		protected override void RenderContents(System.Web.UI.HtmlTextWriter writer)
		{
			...
			if(ErrorMsg.Length == 0)
			{
				...
				if(Editing)
				{
					writer.Write("<b>Editing producttype: " + DB.RSField(rs,"Name") + "<br><br></b>\n");
				}
				else
				{
