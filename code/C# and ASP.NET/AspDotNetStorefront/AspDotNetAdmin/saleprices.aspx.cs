...
namespace AspDotNetStorefrontAdmin
{
	...
	/// </summary>
	public class saleprices : SkinBase
	{
		...
		protected override void RenderContents(System.Web.UI.HtmlTextWriter writer)
		{
			...
			writer.Write("<a href=\"saleprices.aspx?categoryid=0&sectionid=0&producttypeid=0&manufacturerid=0\">RESET FILTERS</a><br>");
			...
			if(PageNum > NumPages)
			{
				if(NumRows > 0)
				{
					Response.Redirect("saleprices.aspx?categoryid=" + CategoryID.ToString() + "&producttypeid=" + ProductTypeID.ToString() + "&manufacturerID=" + ManufacturerID.ToString() + "&pagenum=" + (PageNum-1).ToString());
				}
			}
			...
			if(PagingOn && NumRows >= PageSize)
			{
				if(Common.AppConfig("PageNumberFormat").ToUpper() == "NEXT_PREV")
				{
					...
					if(NumPages > 1)
					{
						...
						if(PageNum > 1)
						{
							writer.Write("<a href=\"saleprices.aspx?" + QueryParms + "&pagenum=1\">First Page</a>");
							writer.Write(" | ");
							writer.Write("<a class=\"PageNumber\" href=\"saleprices.aspx?" + QueryParms + "&pagenum=" + (PageNum-1).ToString() + "\">Previous Page</a>");
						}
						...
						if(PageNum < NumPages)
						{
							writer.Write("<a class=\"PageNumber\" href=\"saleprices.aspx?" + QueryParms + "&pagenum=" + (PageNum+1).ToString() + "\">Next Page</a>");
							writer.Write(" | ");
							writer.Write("<a href=\"saleprices.aspx?" + QueryParms + "&pagenum=" + NumPages.ToString() + "\">Last Page</a>");
						}
						writer.Write(")");
						writer.Write("&nbsp;&nbsp;&nbsp;&nbsp;Click <a class=\"PageNumber\" href=\"saleprices.aspx?" + QueryParms + "&show=all\">here</a> to see all items");
					}
				}
				else
				{
			...
			writer.Write("<form id=\"PriceForm\" name=\"PriceForm\" method=\"POST\" action=\"saleprices.aspx?categoryid=" + CategoryID.ToString() + "&producttypeid=" + ProductTypeID.ToString() + "&manufacturerID=" + ManufacturerID.ToString() + "&pagenum=" + PageNum.ToString() + "\" onsubmit=\"return (validateForm(this) && PriceForm_Validator(this))\" >\n");
